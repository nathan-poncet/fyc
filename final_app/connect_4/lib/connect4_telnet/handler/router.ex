defmodule Connect4Telnet.Handler.Router do
  ...
  def route(%Conn{path: [:game]} = conn) do
    ...
    3. funfacts --number=<number>           | (Get random fun facts)
  ...
  def route(%Conn{path: [:game, :actions, :create], params: %{"name" => name}} = conn) do
    ...
    case Games.game_create(player, name, %{board: %{cols: 7, rows: 6}}) do
      {:ok, code} ->
        Conn.exec(conn.pid, fn -> PubSub.subscribe("game:#{code}") end)
        %Conn{conn | path: [:game, code], response: "Game #{code} created!", status: :ok}
  ...
  def route(%Conn{path: [:game, _game_code, :actions]} = conn) do
    conn |> route_not_found()
  end
  ...
  def route(
    ...
    case Games.game_move(game_code, pid, col_index) do
      {:ok, _} ->
        %Conn{conn | path: [:game, game_code, :actions], response: "", status: :ok}
  ...
  def route(%Conn{path: [:game, game_code, :actions, :leave], pid: pid} = conn) do
    case Games.game_leave(game_code, pid) do
      {:ok, game} ->
        ...
        Conn.exec(pid, fn -> PubSub.unsubscribe("game:#{code}") end)

        %Conn{conn | path: [:game], response: "Game left succefully", status: :ok}
  ...
  defp route_not_found(%Conn{} = conn) do
    index = Enum.find_index(conn.path, fn x -> x == :actions end)
    new_path = Enum.take(conn.path, index + 1)
    %Conn{conn | path: new_path, response: "Command not found"}
  end
end
