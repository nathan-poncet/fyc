defmodule Connect4.Games.Server do
  ...
  def handle_call({:leave, player_id}, _from, state) do
    ...
    PubSub.broadcast("game:#{state.code}", {:leave, player_id, state})
    {:reply, {:ok, new_state}, new_state}
  end
  ...
  @doc """
  This callback is invoked by a monitored process when it dies.
  """
  def handle_info({:DOWN, _ref, :process, pid, _info}, state) do
    new_state = Game.leave(state, pid)
    PubSub.broadcast("game:#{state.code}", {:leave, pid, state})
    {:noreply, new_state}
  end
