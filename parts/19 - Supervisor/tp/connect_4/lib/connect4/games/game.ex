defmodule Connect4.Games.Game do
  ...
  iex> Connect4.Games.Game.create(%Connect4.Games.Player{id: "Player 1", symbol: "X"}, "Code", "Game 1", %{board: %{cols: 7, rows: 6}})
  %Connect4.Games.Game{
    board:
      [
    ...
    settings: %{board: %{cols: 7, rows: 6}, winning_length: 4},
    status: :lobby
  }
  """
  def create(%Player{} = player, code, name, settings) do
    ...
    %Game{
      ...
      board: board,
      players: [player],
      settings: %{board: %{cols: cols, rows: rows}, winning_length: 4},
      status: :lobby
    }
  end
