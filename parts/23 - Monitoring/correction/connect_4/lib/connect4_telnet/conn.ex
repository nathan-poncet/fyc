defmodule Connect4Telnet.Conn do
  ...
  def handle_info({:broadcast, {:lobby, %Game{} = game}}, state) do
    res = """
    Go back to lobby.

    Game #{game.name} is waiting for players to join.

    --- Tap "join --code=<game_code> --symbol=<player_symbol>" to join. ---
    """
  ...
  @doc """
  Handle process down messages.
  """
  def handle_info({:EXIT, _pid, {{:badmatch, {:error, :enotconn}}, _}}, state) do
    {:stop, :normal, state}
  end

  def handle_info({:EXIT, _pid, reason}, state) do
    Logger.info("EXIT: Handling disconnected ref in process with reason #{inspect(reason)}")

    send_response(%__MODULE__{state | response: "An unexpected error happened. Please try again."})
