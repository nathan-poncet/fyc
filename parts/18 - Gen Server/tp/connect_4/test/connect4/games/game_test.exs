defmodule Connect4.Games.GameTest do
  ...
  test "create" do
    ...
    assert %Game{board: ^expected_board, status: :lobby} =
             Connect4.Games.Game.create(player, code, name, settings)
  end
  ...
  test "create with cols and rows settings negative value" do
    ...
    %Game{settings: %{board: %{cols: cols, rows: rows}}, status: :lobby} =
      Connect4.Games.Game.create(player, code, name, settings)
  ...
  test "start" do
    assert {:ok, %Game{status: :playing}} = Connect4.Games.Game.start(%Game{status: :lobby})
  end

  test "start with wrong status" do
    assert {:error, :wrong_status} = Connect4.Games.Game.start(%Game{status: :playing})
  end

  test "lobby" do
    assert {:ok, %Game{status: :lobby}} = Connect4.Games.Game.lobby(%Game{status: :game_over})
  end

  test "lobby with wrong status" do
    assert {:error, :wrong_status} = Connect4.Games.Game.lobby(%Game{status: :playing})
  end

  test "move" do
    player_1_id = "Player 1"
    player_1 = %Player{id: player_1_id, symbol: "X"}
    player_2_id = "Player 2"
    player_2 = %Player{id: player_2_id, symbol: "O"}
  ...
  test "move with wrong status" do
    player_id = "Player 1"
    player = %Player{id: player_id, symbol: "X"}
